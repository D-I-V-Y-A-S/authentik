...
def restore_access_user_bartech(account_id):
    try:
        response = requests.post(f"{JIRA_URL_BARTECH}/{account_id}/restore", headers=headers_1)
        if response.status_code == 204:
            return "Done"
        else:
            print(f"Failed to deactivate user {account_id}: {response.status_code} {response.text}")
...
def restore_access_user_foocorp(account_id):
    try:
        response = requests.post(f"{JIRA_URL_FOOCORP}/{account_id}/restore", headers=headers_1)
        if response.status_code == 204:
            return "Done"
        else:
            print(f"Failed to deactivate user {account_id}: {response.status_code} {response.text}")
                ...
                try:
                    ...
                    if user_data:
                        ...
                        if account_status == "active" and is_active:
                            ...
                            print("all good!")
                        elif account_status == "suspended" and is_active:
                            print("Need to restore user access in Jira")
                            print(restore_access_user_bartech(account_id))
                ...
                try:
                    ...
                    if user_data:
                        ...
                        if account_status == "active" and is_active:
                            ...
                            print("Need to restore user access in Jira")
                            print(restore_access_user_foocorp(account_id))
                        elif account_status == "active" and not is_active:
                            print("Need to suspend user access in Jira")
...
# # Example run
# compare_user(username_to_search="Test_User 29")
